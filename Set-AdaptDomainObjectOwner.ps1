...
function Set-AdaptDomainObjectOwner {
    ...
    BEGIN {
        ...
        if ($PSBoundParameters['Tombstone']) { $SearcherArguments['Tombstone'] = $Tombstone }
        if ($PSBoundParameters['Credential']) { $SearcherArguments['Credential'] = $Credential }
        $OwnerSid = Get-AdaptDomainObject @SearcherArguments -Identity $OwnerIdentity -Properties objectsid | Select-Object -ExpandProperty objectsid
        if ($OwnerSid) {
            $OwnerIdentityReference = [System.Security.Principal.SecurityIdentifier]$OwnerSid
        }
        else {
            Write-Warning "[Set-AdaptDomainObjectOwner] Error parsing owner identity '$OwnerIdentity'"
        }
    }
    PROCESS {
        if ($OwnerIdentityReference) {
            ...
            ForEach ($Object in $RawObject) {
                try {
                    Write-Verbose "[Set-AdaptDomainObjectOwner] Attempting to set the owner for '$Identity' to '$OwnerIdentity'"
                    ...
                    Write-Warning "[Set-AdaptDomainObjectOwner] Error setting owner: $_"
                }
            }
        }
    }
