...
function Set-AdaptDomainUserPassword {
    ...
    if ($Context) {
        ...
        if ($User) {
            Write-Verbose "[Set-AdaptDomainUserPassword] Attempting to set the password for user '$Identity'"
            try {
                $TempCred = New-Object System.Management.Automation.PSCredential('a', $AccountPassword)
                $User.SetPassword($TempCred.GetNetworkCredential().Password)
                $Null = $User.Save()
                Write-Verbose "[Set-AdaptDomainUserPassword] Password for user '$Identity' successfully reset"
            }
            catch {
                Write-Warning "[Set-AdaptDomainUserPassword] Error setting password for user '$Identity' : $_"
            }
        }
        else {
            Write-Warning "[Set-AdaptDomainUserPassword] Unable to find user '$Identity'"
        }
    }
}
