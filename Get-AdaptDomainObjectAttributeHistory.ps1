...
function Get-AdaptDomainObjectAttributeHistory {
    [CmdletBinding()]
    Param(
        [Parameter(Position = 0, ValueFromPipeline = $True, ValueFromPipelineByPropertyName = $True)]
        [Alias('DistinguishedName', 'SamAccountName', 'Name', 'MemberDistinguishedName', 'MemberName')]
        ...
        Get-AdaptDomainObject @SearcherArguments | ForEach-Object {
            ...
            ForEach($XMLNode in $_.Properties['msds-replattributemetadata']) {
                ...
                if ($TempObject) {
                    ...
                    Write-Verbose "[Get-AdaptDomainObjectAttributeHistory] Error retrieving 'msds-replattributemetadata' for '$ObjectDN'"
                }
            }
        }
    }
