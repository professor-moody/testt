...
function Get-AdaptDomainObjectLinkedAttributeHistory {
    [CmdletBinding()]
    Param(
        [Parameter(Position = 0, ValueFromPipeline = $True, ValueFromPipelineByPropertyName = $True)]
        [Alias('DistinguishedName', 'SamAccountName', 'Name', 'MemberDistinguishedName', 'MemberName')]
        ...
        Get-AdaptDomainObject @SearcherArguments | ForEach-Object {
            ...
            ForEach($XMLNode in $_.Properties['msds-replvaluemetadata']) {
                ...
                if ($TempObject) {
                    ...
                    Write-Verbose "[Get-AdaptDomainObjectLinkedAttributeHistory] Error retrieving 'msds-replvaluemetadata' for '$ObjectDN'"
                }
            }
        }
    }
...
function Get-AdaptDomainGroupMemberDeleted {
    [CmdletBinding()]
    Param(
        [Parameter(Position = 0, ValueFromPipeline = $True, ValueFromPipelineByPropertyName = $True)]
        [Alias('DistinguishedName', 'SamAccountName', 'Name', 'MemberDistinguishedName', 'MemberName')]
        ...
        Get-AdaptDomainObject @SearcherArguments | ForEach-Object {
            ...
            ForEach($XMLNode in $_.Properties['msds-replvaluemetadata']) {
                ...
                if ($TempObject) {
                    ...
                    Write-Verbose "[Get-AdaptDomainGroupMemberDeleted] Error retrieving 'msds-replvaluemetadata' for '$ObjectDN'"
                }
            }
        }
    }
