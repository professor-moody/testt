...
function Get-AdaptWMIRegMountedDrive {
    [CmdletBinding()]
    Param(
        [Parameter(Position = 0, ValueFromPipeline = $True, ValueFromPipelineByPropertyName = $True)]
            ...
            try {
                ...
                ForEach ($UserSID in $UserSIDs) {
                    try {
                        ...
                        Write-Verbose "[Get-AdaptWMIRegMountedDrive] Error: $_"
                    }
                }
            }
            catch {
                Write-Warning "[Get-AdaptWMIRegMountedDrive] Error accessing $Computer, likely insufficient permissions or firewall rules on host: $_"
            }
        }
    }
}
