...
function Get-AdaptDomainGUIDMap {
    ...
    try {
        $SchemaPath = (Get-AdaptForest @ForestArguments).schema.name
    }
    catch {
        throw '[Get-AdaptDomainGUIDMap] Error in retrieving forest schema path from Get-AdaptForest'
    }
    if (-not $SchemaPath) {
        throw '[Get-AdaptDomainGUIDMap] Error in retrieving forest schema path from Get-AdaptForest'
    }
        ...
        try {
            ...
            if ($Results) {
                try { $Results.dispose() }
                    ...
                    Write-Verbose "[Get-AdaptDomainGUIDMap] Error disposing of the Results object: $_"
                }
            }
            $SchemaSearcher.dispose()
        }
        catch {
            Write-Verbose "[Get-AdaptDomainGUIDMap] Error in building GUID map: $_"
        }
    }
...
function Get-AdaptDomainObjectAcl {
    [CmdletBinding()]
    Param (
        [Parameter(Position = 0, ValueFromPipeline = $True, ValueFromPipelineByPropertyName = $True)]
    ...
    BEGIN {
        ...
        if ($PSBoundParameters['ResolveGUIDs']) {
            $GUIDs = Get-AdaptDomainGUIDMap @DomainGUIDMapArguments
        }
    }
    PROCESS {
        if ($Searcher) {
            ...
            $Identity | Where-Object {$_} | ForEach-Object {
                ...
                elseif ($IdentityInstance -match '^(CN|OU|DC)=.*') {
                    ...
                    if ((-not $PSBoundParameters['Domain']) -and (-not $PSBoundParameters['SearchBase'])) {
                        $IdentityDomain = $IdentityInstance.SubString($IdentityInstance.IndexOf('DC=')) -replace 'DC=','' -replace ',','.'
                        Write-Verbose "[Get-AdaptDomainObjectAcl] Extracted domain '$IdentityDomain' from '$IdentityInstance'"
                        $SearcherArguments['Domain'] = $IdentityDomain
                        $Searcher = Get-DomainSearcher @SearcherArguments
                        if (-not $Searcher) {
                            Write-Warning "[Get-AdaptDomainObjectAcl] Unable to retrieve domain searcher for '$IdentityDomain'"
                        }
                    }
                }
                elseif ($IdentityInstance -imatch '^[0-9A-F]{8}-([0-9A-F]{4}-){3}[0-9A-F]{12}$') {
            ...
            if ($PSBoundParameters['LDAPFilter']) {
                Write-Verbose "[Get-AdaptDomainObjectAcl] Using additional LDAP filter: $LDAPFilter"
                $Filter += "$LDAPFilter"
            }
            if ($Filter) {
                $Searcher.filter = "(&$Filter)"
            }
            Write-Verbose "[Get-AdaptDomainObjectAcl] Get-AdaptDomainObjectAcl filter string: $($Searcher.filter)"
            ...
            $Results | Where-Object {$_} | ForEach-Object {
                ...
                try {
                    ...
                    Write-Verbose "[Get-AdaptDomainObjectAcl] Error: $_"
                }
            }
        }
    }
...
function Get-AdaptForest {
    ...
    PROCESS {
        if ($PSBoundParameters['Credential']) {
            Write-Verbose "[Get-AdaptForest] Using alternate credentials for Get-AdaptForest"
            if ($PSBoundParameters['Forest']) {
                $TargetForest = $Forest
            }
            else {
                $TargetForest = $Credential.GetNetworkCredential().Domain
                Write-Verbose "[Get-AdaptForest] Extracted domain '$Forest' from -Credential"
            }
            ...
            try {
                ...
                Write-Verbose "[Get-AdaptForest] The specified forest '$TargetForest' does not exist, could not be contacted, there isn't an existing trust, or the specified credentials are invalid: $_"
                $Null
            }
        }
        elseif ($PSBoundParameters['Forest']) {
            ...
            try {
                ...
                Write-Verbose "[Get-AdaptForest] The specified forest '$Forest' does not exist, could not be contacted, or there isn't an existing trust: $_"
                return $Null
            }
        }
        else {
        ...
        if ($ForestObject) {
            if ($PSBoundParameters['Credential']) {
                $ForestSid = (Get-AdaptDomainUser -Identity "krbtgt" -Domain $ForestObject.RootDomain.Name -Credential $Credential).objectsid
            }
            else {
                $ForestSid = (Get-AdaptDomainUser -Identity "krbtgt" -Domain $ForestObject.RootDomain.Name).objectsid
            }
...
function Find-AdaptInterestingDomainAcl {
    [CmdletBinding()]
    Param (
        [Parameter(Position = 0, ValueFromPipeline = $True, ValueFromPipelineByPropertyName = $True)]
    ...
    PROCESS {
        ...
        Get-AdaptDomainObjectAcl @ACLArguments | ForEach-Object {
                ...
                if ($_.SecurityIdentifier.Value -match '^S-1-5-.*-[1-9]\d{3,}$') {
                    if ($ResolvedSIDs[$_.SecurityIdentifier.Value]) {
                        ...
                        if ($IdentityReferenceDN) {
                            ...
                            Write-Warning "[Find-AdaptInterestingDomainAcl] Unable to convert SID '$($_.SecurityIdentifier.Value )' to a distinguishedname with Convert-ADName"
                        }
                    }
                }
            }
