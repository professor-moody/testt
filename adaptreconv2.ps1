...
.EXAMPLE
  ...
  # Skip the service registry permission scan (can be slow)
  .\adaptrecon.ps1 -SkipRegScan
...
if ($SkipRegScan) {
  Write-Host "Skipped (use without -SkipRegScan to enable)" -ForegroundColor DarkGray
}
else {
  Write-Host "Scanning service registry keys (this may take a while)..."
  $regServices = Get-ChildItem 'HKLM:\System\CurrentControlSet\services\' -ErrorAction SilentlyContinue
  $regTotal = $regServices.Count
  $regCount = 0
  $regServices | ForEach-Object {
    $regCount++
    if ($regCount % 50 -eq 0) {
      Write-Host "  Progress: $regCount / $regTotal services checked..." -ForegroundColor DarkGray
    }
    $target = $_.Name.Replace("HKEY_LOCAL_MACHINE", "hklm:")
    Test-PathPermissions -Target $target
  }
  Write-Host "  Completed: $regTotal services checked" -ForegroundColor DarkGray
}
