...
function Add-AdaptDomainObjectAcl {
    [CmdletBinding()]
    Param (
    ...
    BEGIN {
        ...
        $Principals = Get-AdaptDomainObject @PrincipalSearcherArguments
        if (-not $Principals) {
    ...
    PROCESS {
        ...
        $Targets = Get-AdaptDomainObject @TargetSearcherArguments
        ForEach ($TargetObject in $Targets) {
            ...
            ForEach ($PrincipalObject in $Principals) {
                Write-Verbose "[Add-AdaptDomainObjectAcl] Granting principal $($PrincipalObject.distinguishedname) '$Rights' on $($TargetObject.Properties.distinguishedname)"
                try {
                    ...
                    ForEach ($ACE in $ACEs) {
                        Write-Verbose "[Add-AdaptDomainObjectAcl] Granting principal $($PrincipalObject.distinguishedname) rights GUID '$($ACE.ObjectType)' on $($TargetObject.Properties.distinguishedname)"
                    ...
                    Write-Verbose "[Add-AdaptDomainObjectAcl] Error granting principal $($PrincipalObject.distinguishedname) '$Rights' on $($TargetObject.Properties.distinguishedname) : $_"
                }
            }
        }
    }
...
function Remove-AdaptDomainObjectAcl {
    [CmdletBinding()]
    Param (
        [Parameter(Position = 0, ValueFromPipeline = $True, ValueFromPipelineByPropertyName = $True)]
        [Alias('DistinguishedName', 'SamAccountName', 'Name')]
    ...
    PROCESS {
        ...
        ForEach ($TargetObject in $Targets) {
            ...
            ForEach ($PrincipalObject in $Principals) {
                Write-Verbose "[Remove-AdaptDomainObjectAcl] Removing principal $($PrincipalObject.distinguishedname) '$Rights' from $($TargetObject.Properties.distinguishedname)"
                try {
                    $Identity = [System.Security.Principal.IdentityReference] ([System.Security.Principal.SecurityIdentifier]$PrincipalObject.objectsid)
                    if ($GUIDs) {
                        ForEach ($GUID in $GUIDs) {
                    ...
                    ForEach ($ACE in $ACEs) {
                        Write-Verbose "[Remove-AdaptDomainObjectAcl] Granting principal $($PrincipalObject.distinguishedname) rights GUID '$($ACE.ObjectType)' on $($TargetObject.Properties.distinguishedname)"
                        $TargetEntry = $TargetObject.GetDirectoryEntry()
                        $TargetEntry.PsBase.Options.SecurityMasks = 'Dacl'
                    ...
                    Write-Verbose "[Remove-AdaptDomainObjectAcl] Error removing principal $($PrincipalObject.distinguishedname) '$Rights' from $($TargetObject.Properties.distinguishedname) : $_"
                }
            }
        }
    }
