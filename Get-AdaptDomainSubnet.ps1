...
function Get-AdaptDomainSubnet {
    [CmdletBinding()]
    Param (
    ...
    PROCESS {
        if ($SubnetSearcher) {
            ...
            $Identity | Where-Object {$_} | ForEach-Object {
                ...
                if ($IdentityInstance -match '^CN=.*') {
                    ...
                    if ((-not $PSBoundParameters['Domain']) -and (-not $PSBoundParameters['SearchBase'])) {
                        ...
                        Write-Verbose "[Get-AdaptDomainSubnet] Extracted domain '$IdentityDomain' from '$IdentityInstance'"
                        $SearcherArguments['Domain'] = $IdentityDomain
                        $SubnetSearcher = Get-DomainSearcher @SearcherArguments
                        if (-not $SubnetSearcher) {
                            Write-Warning "[Get-AdaptDomainSubnet] Unable to retrieve domain searcher for '$IdentityDomain'"
                        }
                    }
                }
                else {
            ...
            if ($PSBoundParameters['LDAPFilter']) {
                Write-Verbose "[Get-AdaptDomainSubnet] Using additional LDAP filter: $LDAPFilter"
                $Filter += "$LDAPFilter"
            }
            $SubnetSearcher.filter = "(&(objectCategory=subnet)$Filter)"
            Write-Verbose "[Get-AdaptDomainSubnet] Get-AdaptDomainSubnet filter string: $($SubnetSearcher.filter)"
            ...
            if ($Results) {
                try { $Results.dispose() }
                    ...
                    Write-Verbose "[Get-AdaptDomainSubnet] Error disposing of the Results object: $_"
                }
            }
