...
function New-AdaptDomainGroup {
    ...
    if ($Context) {
        ...
        Write-Verbose "[New-AdaptDomainGroup] Attempting to create group '$SamAccountName'"
        try {
            $Null = $Group.Save()
            Write-Verbose "[New-AdaptDomainGroup] Group '$SamAccountName' successfully created"
            $Group
        }
        catch {
            Write-Warning "[New-AdaptDomainGroup] Error creating group '$SamAccountName' : $_"
        }
    }
}
