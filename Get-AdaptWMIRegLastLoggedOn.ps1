...
function Get-AdaptWMIRegLastLoggedOn {
    [CmdletBinding()]
    Param(
        [Parameter(Position = 0, ValueFromPipeline = $True, ValueFromPipelineByPropertyName = $True)]
    ...
    PROCESS {
        ForEach ($Computer in $ComputerName) {
            ...
            try {
                ...
                Write-Warning "[Get-AdaptWMIRegLastLoggedOn] Error opening remote registry on $Computer. Remote registry likely not enabled."
            }
        }
    }
}
