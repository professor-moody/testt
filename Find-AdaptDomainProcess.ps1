...
function Find-AdaptDomainProcess {
    ...
    BEGIN {
        ...
        if ($PSBoundParameters['ComputerName']) {
            ...
            Write-Verbose '[Find-AdaptDomainProcess] Querying computers in the domain'
            $TargetComputers = Get-AdaptDomainComputer @ComputerSearcherArguments | Select-Object -ExpandProperty dnshostname
        }
        Write-Verbose "[Find-AdaptDomainProcess] TargetComputers length: $($TargetComputers.Length)"
        if ($TargetComputers.Length -eq 0) {
            throw '[Find-AdaptDomainProcess] No hosts found to enumerate'
        }
        ...
        $HostEnumBlock = {
            ...
            ForEach ($TargetComputer in $ComputerName) {
                ...
                if ($Up) {
                    if ($Credential) {
                        $Processes = Get-AdaptWMIProcess -Credential $Credential -ComputerName $TargetComputer -ErrorAction SilentlyContinue
                    }
                    else {
                        $Processes = Get-AdaptWMIProcess -ComputerName $TargetComputer -ErrorAction SilentlyContinue
                    }
    ...
    PROCESS {
        if ($PSBoundParameters['Delay'] -or $PSBoundParameters['StopOnSuccess']) {
            Write-Verbose "[Find-AdaptDomainProcess] Total number of hosts: $($TargetComputers.count)"
            Write-Verbose "[Find-AdaptDomainProcess] Delay: $Delay, Jitter: $Jitter"
            $Counter = 0
            $RandNo = New-Object System.Random
            ForEach ($TargetComputer in $TargetComputers) {
                $Counter = $Counter + 1
                Start-Sleep -Seconds $RandNo.Next((1-$Jitter)*$Delay, (1+$Jitter)*$Delay)
                Write-Verbose "[Find-AdaptDomainProcess] Enumerating server $TargetComputer ($Counter of $($TargetComputers.count))"
                ...
                if ($Result -and $StopOnSuccess) {
                    Write-Verbose "[Find-AdaptDomainProcess] Target user found, returning early"
                    return
                }
            }
        }
        else {
            Write-Verbose "[Find-AdaptDomainProcess] Using threading with threads: $Threads"
            $ScriptParams = @{
