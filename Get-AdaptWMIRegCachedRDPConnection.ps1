...
function Get-AdaptWMIRegCachedRDPConnection {
    [CmdletBinding()]
    Param(
        [Parameter(Position = 0, ValueFromPipeline = $True, ValueFromPipelineByPropertyName = $True)]
            ...
            try {
                ...
                ForEach ($UserSID in $UserSIDs) {
                    try {
                        ...
                        Write-Verbose "[Get-AdaptWMIRegCachedRDPConnection] Error: $_"
                    }
                }
            }
            catch {
                Write-Warning "[Get-AdaptWMIRegCachedRDPConnection] Error accessing $Computer, likely insufficient permissions or firewall rules on host: $_"
            }
        }
    }
}
